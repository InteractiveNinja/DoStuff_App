...
public interface TaskRepository extends CrudRepository<TaskDAO, Integer> {
    ...
    @Query(value = "SELECT * FROM task t where DATE(t.zuerledigen) >= CURDATE() and DATE(t.zuerledigen) < CURDATE()+1 order by t.erledigt, t.zuerledigen",nativeQuery = true)
    public Set<TaskDAO> getAllToday();

    // Hole alle von heute die keine wiederholung haben
    @Query(value = "SELECT * FROM task t where DATE(t.zuerledigen) >= CURDATE() and  DATE(t.zuerledigen) < CURDATE()+1 and t.wiederholungen_id = 1",nativeQuery = true)
    public Set<TaskDAO> getAllWithRepeatNever();

    // Hole alle von heute die eine Tägliche wiederholung haben
    @Query(value = "SELECT * FROM task t where DATE(t.zuerledigen) >= CURDATE() and  DATE(t.zuerledigen) < CURDATE()+1 and t.wiederholungen_id = 2",nativeQuery = true)
    public Set<TaskDAO> getAllWithRepeatDaily();

    // Hole alle von heute die eine Wöchentliche wiederholung haben
    @Query(value = "SELECT * FROM task t where DATE(t.zuerledigen) >= CURDATE() and  DATE(t.zuerledigen) < CURDATE()+1 and t.wiederholungen_id = 3",nativeQuery = true)
    public Set<TaskDAO> getAllWithRepeatWeekly();

    // Hole alle die Älter als Heute sind
    @Query(value = "SELECT * FROM task t where DATE(t.zuerledigen) < CURDATE()",nativeQuery = true)
